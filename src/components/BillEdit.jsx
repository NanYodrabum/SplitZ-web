...
function BillEdit() {
  ...
  useEffect(() => {
    const fetchBillDetails = async () => {
      try {
        ...
        // Set bill items - force ids to start at 1
        if (bill.items && bill.items.length > 0) {
          setBillItems(bill.items.map((item, index) => {
            // Force item id to be index + 1 (starting from 1 instead of 20)
            const itemId = index + 1;
            
            // Get participant IDs who are part of this item's splits
            const splitParticipantIds = item.splits?.map(split => {
              return split.billParticipantId || 
                    (split.participant && split.participant.id) || 
                    split.participantId;
            }) || [];
            
            return {
              id: itemId,
              originalId: item.id, // Keep original ID for API calls
              name: item.name,
              basePrice: item.basePrice,
              taxPercent: item.taxPercent || 7,
  ...
  const addBillItem = () => {
    const newId = billItems.length > 0
      ? billItems.length + 1 // Always increment by 1 from the last item
      : 1;
    ...
    setBillItems([
      ...billItems,
      {
        ...
        splitWith: participants.length > 0 ? [participants[0].id] : []
      }
    ]);
  };
  ...
  const removeBillItem = (id) => {
    ...
    // Remove the item
    const filteredItems = billItems.filter(item => item.id !== id);
  ...
  // Calculate service charge amount for display
  ...
  const handleSubmit = async () => {
    try {
      ...
      const processedItems = billItems.map(item => {
        ...
        return {
          id: item.originalId || item.id, // Use original ID if available
          name: item.name,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
              ...
              <div className="space-y-3 mb-4">
                {participants.map((participant) => (
                  <div key={participant.id} className="flex items-center">
                    ...
                    />
                    <button
                      className={`ml-2 text-red-500 p-2 hover:text-red-700 ${
            ...
            <div className="space-y-4">
              ...
              {billItems.map((item) => (
                <div key={item.id} className="bg-white rounded-lg border p-4 relative">
                  <div className="absolute top-4 left-4 bg-purple-600 text-white w-8 h-8 rounded-md flex items-center justify-center">
                    {item.id}
                  </div>
              ...
              <button
                className="w-full p-4 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:border-blue-300 hover:text-blue-500 flex items-center justify-center"
                onClick={addBillItem}
              >
                <Plus size={20} />
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg border p-4 lg:sticky lg:top-4">
              ...
              {/* Base, Tax, Service breakdowns */}
              <div className="space-y-2 mb-4">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Base Total</span>
                  <span className="font-medium">
                    ${Math.round(billItems.reduce((sum, item) => sum + (parseFloat(item.basePrice) || 0), 0))}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Tax Total</span>
                  <span className="font-medium">
                    ${Math.round(billItems.reduce((sum, item) => sum + calculateTaxAmount(item.basePrice, item.taxPercent), 0))}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Service Total</span>
                  <span className="font-medium">
                    ${Math.round(billItems.reduce((sum, item) => sum + calculateServiceAmount(item.basePrice, item.serviceChargePercent), 0))}
                  </span>
                </div>
              </div>
              
              {/* Grand Total */}
              <div className="pt-3 border-t mb-6">
              ...
              <div className="space-y-3 mb-6">
                {getParticipantSummary().map((summary) => (
                  <div key={summary.name} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    ...
                    <span className="font-semibold text-right">${Math.round(summary.amount)}</span>
                  </div>
                ))}
              </div>
